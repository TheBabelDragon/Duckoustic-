...
bool LaserOutput::begin() {
    ...
    // ESP32 Arduino LEDC API (v3.x / IDF 5 style)
    // Attach pin to channel with frequency and resolution.
    if (!ledcAttach(DUCK_LASER_PWM_PIN, DUCK_LASER_PWM_FREQ_HZ, DUCK_LASER_PWM_RES_BITS)) {
        // Fallback for older cores that still use ledcSetup + ledcAttachPin
